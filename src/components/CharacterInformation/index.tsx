...
const CharacterInformation: React.FC<CharacterInformationProps> = ({
  ...
  const Luffy =
    'É o capitão do Bando do Chapeu de Palha, filho de Dragon, o homem mais procurado do mundo e neto de Monkey D. Garp, Vice-Almirante da Marinha e arqui-inimigo de Gol D. Roger, que fez história no mundo como o Rei dos Piratas e o dono original do tesouro One Piece. Atualmente, Luffy faz parte do grupo conhecido como "Pior Geração", que reconhece os 12 piratas cujas ousadas ações contra o Governo Mundial lhe renderam uma recompensa acima de $100.000.000. A do Luffy é $1.500.000.000.';

  const Zoro =
    'O espadachim que domina os estilos Ittoryu (Estilo Uma Espada), o Nitoryu (Estilo Duas Espadas), e desenvolveu por conta própria o Santoryu (Estilo Três Espadas). foi o primeiro a entrar no bando, e assim como seu capitão, faz parte da Pior Geração com uma recompensa de $320.000.000. Enquanto Luffy sonha em se tornar o Rei dos Piratas, o sonho de Zoro é um dia ser o melhor espadachim do mundo ,sonho que adiquiriu durante a infancia apos a morte de sua unica amiga, que sofria preconceito por ser mulher e querer ser espadachim.';

  const Nami =
    'Apos passar 8 anos trabalhando forçadamente para Arlong, que tinha todo seu vilarejo como refens, recebeu a missão de tentar roubar os Chapéus de Palha. Para sua surpresa, a tripulação de Luffy conseguiu se rebelar e vencer Arlong. É a cartografa e navegadora do navio, graças ao seu grande conhecimento e com a ajuda de uma das ferramentas criadas por Usopp, é capas de mudar o clima e até criar tempestades. E apesar de não estar entre os mais fortes do barco é a pessoa que assume a liderança do barco na auxencia do capitão, assim como um imediato faria (apesar de não haver ninguem com este posto no bando)';

  const Usopp =
    'inicialmente é introduzido apenas como um garoto famozo por suas mentiras. Contudo, o pirata é um homem sensível, e suas narrativas incríveis surgiram como mecanismo de defesa. São uma forma que ele encontrou, ainda criança, de lidar com a morte da mãe e o abandono do pai. Mas apos ser convidado para entrar no bando se mostra estremamente engenhoso e inventivo, alem de ser um eximio atirador e filho do melhor atirador do mundo, apesar de ser extremamente medroso se mostra um guerreiro poderoso quando necessario';

  const Sanji =
    'Apesar de ser filho da nobre familia Vinsmoke, durante a infancia sofreu bullying de seus irmaos menosprezando sua atitude de alimentar aqueles que seu pai julgava inferiores e a quem não dava atenção. O patriarca via o filho como a vergonha da família. Sanji renunciou seu título de príncipe do Reino Germa. Foi convidado para o bando quando Luffy o viu entrar em uma briga com um tenente da marinha por desperdiçar comida, e alimetar um fugitivo faminto as escondidas. Alem de ser o cozinheiro do barco é um ezimio lutador ficando entre os três integrantes mais fortes do grupo, é uma pessoa séria e sensata a maior parte do tempo, exceto quando esta perto de uma mulher bonita';
  ...
  const Robin =
    'Embora tenha sido introduzida como uma antagonista acabou se juntando ao bando apos luffy salvar sua vida e ganhou a confiaça dos resto do grupo apos tentar se entregar a marinha para poupar os outro tripulantes. Ela não é apenas uma arqueologa e historiadora que possui conhecimentos estramemnte valiosos como também a unica sobrevivente da ilha de Ohara e a última pessoa pessoa no mundo capaz de ler os Poneglyphs, ato considerado um risco para o Governo Mundial ';

  const Franky =
    'Assim como Nico Robin, o personagem começa sua trajetória como um antagonista. Um ciborgue de 36 anos, Franky é o líder da Família Franky, que trabalhava com o desmantelamento de navios. É um construtor naval de primeira e um dos integrantes mais atenciosos e maduros, embora seu jeito meio excêntrico passe a ideia errada inicialmente, ele arrisca sua vida para proteger a equipe e quaisquer pessoas que são perseguidas, uma vez que, para ele, ninguém deve ser julgado por ser quem é.';
